...
def render_html(rep: Report, chart_png: bytes) -> str:
    ...
    gen = datetime.fromtimestamp(rep.generated_at).strftime("%d/%m/%Y %H:%M:%S")
    return f"""<!doctype html>
<html lang="pt-br"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>PacketLizer - Relatorio {rep.target}</title>
<style>
 ...
 @media (prefers-color-scheme: dark) {{
   ...
   .card .k, .card .s {{ color:#9ca3af; }}
 }}
</style></head><body>
<header>
  <h1>PacketLizer &mdash; Relatorio de estabilidade</h1>
  <p>Alvo: <b>{rep.target}</b> &nbsp;|&nbsp; Janela: {_fmt_ts(rep.first_ts)} &rarr; {_fmt_ts(rep.last_ts)}
     &nbsp;|&nbsp; {rep.total} amostras &nbsp;|&nbsp; intervalo {rep.interval_seconds:.0f}s
     &nbsp;|&nbsp; gerado em {gen}</p>
</header>
<main>
  <div class="grid">{cards}</div>

  <h2>Latencia ao longo do tempo</h2>
  <img alt="Grafico de latencia" src="data:image/png;base64,{chart_b64}">

  <h2>Quedas detectadas ({rep.outage_count})</h2>
...
# PDF
# ---------------------------------------------------------------------------
def render_pdf(rep: Report, chart_png: bytes, out_path: Path) -> None:
    ...
    story.append(Paragraph(
        f"Alvo: <b>{rep.target}</b><br/>"
        f"Janela: {_fmt_ts(rep.first_ts)} &rarr; {_fmt_ts(rep.last_ts)}<br/>"
        f"{rep.total} amostras, intervalo {rep.interval_seconds:.0f}s<br/>"
        f"Gerado em {datetime.fromtimestamp(rep.generated_at).strftime('%d/%m/%Y %H:%M:%S')}",
        styles["Normal"]))
    story.append(Spacer(1, 0.4 * cm))

    kpi = _kpi_cards(rep)
    data = [[k, v] for k, v, _ in kpi]
...
def export_csv(cfg: Config, out_path: Path, *, days=None, since=None, until=None) -> int:
    ...
    with Storage(cfg.resolved_db_path()) as st, out_path.open("w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
        w.writerow(["timestamp_iso", "timestamp_epoch", "rtt_ms", "status_code", "status"])
        for s in st.iter_samples(start, end):
            iso = datetime.fromtimestamp(s.ts).isoformat()
            w.writerow([iso, s.ts, "" if s.rtt_ms is None else f"{s.rtt_ms:.3f}",
